...
export async function upsertReservations({ reservas, prop, dryRun = false, sourceTag = 'wubookImport', log = console.log }) {
  ...
  for (const r of reservas) {
    const customerName = `${r?.customer?.name || ''} ${r?.customer?.surname || ''}`.trim();
    const bookerId = r?.booker ?? r?.customer?.id ?? r?.customer?.customer_id ?? null;
    ...
    for (const room of r?.rooms || []) {
      ...
      const oldGuestName = String(oldDoc?.nombre_huesped || '').trim();
      const invalidGuestNames = ['undefined', 'null', 'n/d', 'sin nombre', 'error al buscar nombre'];
      const hasUsefulOldGuestName = oldGuestName && !invalidGuestNames.includes(oldGuestName.toLowerCase());
      const fullName = customerName || (hasUsefulOldGuestName ? oldGuestName : '') || (bookerId != null ? String(bookerId) : 'N/D');
      ...
      const baseDoc = {
        ...
        ...(bookerId != null ? { wubook_booker_id: String(bookerId) } : {}),
